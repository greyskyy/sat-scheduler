...
def doWork(vm, callable, test_mode: bool = False):
    """
...
def scheduler(args=None, config=None, vm=None):
    ...
    preprocessingResults = pre_processor.execute(
        args=args, vm=vm, config=config, context=context
    )
