...
class OrekitUtils:
    """Collection of utility classes for creating orekit classes."""
    ...
    def frame(
        ...
        **kwargs,
        ...
        if s.lower() == "j2000" or s.lower() == "eme2000":
            ...
            iers = OrekitUtils.loadIersConventions(iersConventions, "iers_2010")
            return context.getFrames().getITRF(iers, simpleEop)
        else:
            raise ValueError(f"unknown frame type: {s}")

    @staticmethod
    def referenceEllipsoid(
        model: str = "wgs84", frameName: str = "itrf", **kwargs
    ) -> ReferenceEllipsoid:
        ...
        f = OrekitUtils.frame(frameName, **kwargs)
        lModel = model.lower()
        if lModel == "wgs84" or lModel == "wgs-84":
            ...
            raise ValueError(
                f"Cannot convert unknown reference ellipsoid value {model}"
            )

    @staticmethod
    def toVector(
        x: float | list[float] | None, y: float = 0.0, z: float = 0.0
    ) -> Vector3D:
        """Create a Vector3D instance from the values.

        Args:
            x (float | list[float] | None): If a scalar, the x value. Otherwise the [x, y, z] vector.
    ...
    @staticmethod
    def toRotation(
        x: Vector3D = None, y: Vector3D = None, z: Vector3D = None
    ) -> Rotation:
