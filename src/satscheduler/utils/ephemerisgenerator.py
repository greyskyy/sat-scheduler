...
class EphemerisGenerator:
    ...
    def propagate(self, interval:DateInterval, step:float|timedelta=timedelta(minutes=1)):
    ...
    def build(self) -> Ephemeris:
        result = Ephemeris(self.__states, int(2), float(0.001), InertialProvider.of(self.__propagator.getFrame()))
        self.__states = ArrayList()
        return result
    
    @staticmethod
    def generate(propagator:Propagator, interval:DateInterval, step:float|timedelta=timedelta(minutes=1)) -> Ephemeris:
        ...
        return Ephemeris(states, int(2), float(0.001), InertialProvider.of(propagator.getFrame()))